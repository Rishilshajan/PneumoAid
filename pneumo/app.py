...
app.config['MAIL_USERNAME'] = 'rishilshajan4@gmail.com'
...
# Load VGG19 model - rebuild architecture and load weights

# (avoids Keras version compatibility issues with load_model)

MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'vgg19_trained_model.h5')
...
vggmodel.load_weights(MODEL_PATH, by_name=False, skip_mismatch=False)
...
def is_lung_health_question(message):

    """

    Determines whether a message is related to
...
def chatbot():
    ...
    Opens the Pneumo chatbot page.

    """

    # Start a fresh conversation when the page is opened.
    ...
    try:
        ...
        if not session.get("first_message"):
            ...
            greeting = (
                ...
                "Hello! I’m Pneumo, your personal healthcare "

                "companion. I can help you with questions about "

                "pneumonia, lungs, breathing, respiratory health, "

                "symptoms, prevention, and general care."
        ...
        # Check whether the question is relevant
...
# ----------------------------------------------------
        ...
        if not is_lung_health_question(user_message):

            response_text = (

        "I’m Pneumo, a lung-health assistant. "

        "I can help with pneumonia, breathing problems, "

        "lung health, respiratory symptoms, prevention, "
...
You are Pneumo, a friendly healthcare assistant

specialized in lung and respiratory health.

Your responsibilities:

\- Answer the user's actual question directly.

\- Explain lung and respiratory conditions clearly.

\- Explain pneumonia in simple language.

\- Explain common symptoms and causes.

\- Explain prevention and general care.

\- Help users understand medical terminology.

\- Give general health information.

\- Correct obvious spelling or grammar mistakes mentally

  and answer the intended question.
...
\- Encourage the user to consult a qualified healthcare
  ...
  professional when symptoms are serious or persistent.

\- If the user describes severe difficulty breathing,

  blue lips, unconsciousness, severe chest pain,

  or rapidly worsening symptoms, advise them to seek

  emergency medical care immediately.

\- Be calm, friendly and easy to understand.
...
"""
        ...
        if not response_text:
            ...
            response_text = (
                ...
                "to my health-assistant service right now. "
...
def voice_assistant():
    ...
    Opens the Pneumo voice assistant page.
...
def voice():
    ...
    Receives speech-to-text input from the browser

    and generates a spoken-assistant response.
    ...
    try:
        ...
        if not is_lung_health_question(user_message):
            ...
            response_text = (
                ...
                "You can ask me about pneumonia, breathing, "

                "lungs, respiratory symptoms, or prevention."
...
You are Pneumo, a friendly respiratory healthcare assistant.

Answer the user's question naturally as if you are
...
\- Focus on lung and respiratory health when relevant.
...
\- If the user describes severe breathing difficulty,
...
"""
        ...
        if not response_text:
            ...
            response_text = (
                ...
                "Sorry, I'm having trouble processing "
...
def get_clinics():
    clinics = list(db[CLINICS_COL].find({}, {'name': 1}))
    return jsonify([
        {'_id': str(c['_id']), 'name': c['name']}
        for c in clinics
    ])


@app.route('/api/hospitals')  # Backward-compatible endpoint
def get_hospitals():
    return get_clinics()
...
def get_doctors(hospital_id):
    ...
    try:
        ...
        clinic = db[CLINICS_COL].find_one({'_id': ObjectId(hospital_id)})
...
def book_appointment():
    ...
    try:
        ...
        hospital = db[CLINICS_COL].find_one({'_id': ObjectId(data['hospital'])}, {'name': 1})
...
def manage_doctors():
    if request.method == 'POST':
        # Accept clinic_name, while also accepting the old hospital_name
        # field so the current access_code.html keeps working.
        clinic_name = (
            request.form.get('clinic_name')
            or request.form.get('hospital_name', '')
        ).strip()

        access_code = request.form.get('access_code', '').strip()

        clinic = db[CLINICS_COL].find_one({
            'name': clinic_name
        })
        ...
        if access_code != CLINIC_ACCESS_CODE:
            flash('Invalid access code', 'error')
            return redirect(url_for('manage_doctors'))

        # Existing pages use hospital_id/hospital_name, so keep those
        # session keys as compatibility aliases.
        session['clinic_id'] = str(clinic['_id'])
        session['clinic_name'] = clinic.get('name', 'Clinic')
        session['hospital_id'] = str(clinic['_id'])
        session['hospital_name'] = clinic.get('name', 'Clinic')
...
@app.route('/hospital-login', methods=['GET', 'POST'])  # Backward-compatible URL
def hospital_login():
    if request.method == 'POST':
        ...
        if clinic and check_password_hash(
            clinic.get('password', ''),
            password
        ):
            session['clinic_logged_in'] = True
            session['clinic_id'] = str(clinic['_id'])
            session['clinic_name'] = clinic.get('name', 'Clinic')

            # Compatibility aliases for existing clinic dashboard routes.
        ...
        flash('Invalid clinic username or password', 'error')
